...
const Modal = ({ process, onClose }) => {
  ...
  useEffect(() => {
    if (process?.Info) {
      setLoading(true);
      axios.post("https://mysql-backend-1.onrender.com/recommend", { query: process.Info })
...
const ExplainModal = ({ process, onClose }) => {
    ...
    useEffect(() => {
        const fetchExplain = async () => {
            ...
            setLoading(true);
            try {
                const res = await axios.post("https://mysql-backend-1.onrender.com/explain", { query: process.Info });
...
const QueryExecutor = ({ databases }) => {
    ...
    const handleExecute = async () => {
        ...
        setError(null);
        try {
            const res = await axios.post("https://mysql-backend-1.onrender.com/execute", { query, db });
            setResult(res.data);
        } catch (err) {
...
function App() {
  ...
  const fetchProcesses = async () => {
    try {
      setLoading(true);
      const res = await axios.get("https://mysql-backend-1.onrender.com/processes");
  ...
  const fetchDatabases = async () => {
    try {
      const res = await axios.get("https://mysql-backend-1.onrender.com/databases");
  ...
  const killSelected = async () => {
    ...
    if (window.confirm(`⚠️ Kill ${selectedProcesses.length} process(es)?`)) {
      try {
        await axios.post("https://mysql-backend-1.onrender.com/kill", { process_ids: selectedProcesses });
